...
add_cancel_dag = DAG("add_cancel_dag", schedule_interval="40 9-18 * * 1-5")
...
def load_data():
    ...
    files = [x for x in os.listdir("N:\\source_cherkessk\\canceled") if "xlsx" in x]
    logger.info(f"Получено {len(files)} файлов")
    for file in files:
        try:
            df = pd.read_excel(f"N:\\source_cherkessk\\canceled\\{file}")
            ...
            metadata = MetaData()
            table = Table("receipts_cancel", metadata, autoload_with=engine, schema="raw")
